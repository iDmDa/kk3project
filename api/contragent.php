...
function findlist($find) {
    ...
    if(substr_count($find, '#') == 2) {
        ...
        $findlist = "and ((STR_TO_DATE(datestart, '%d.%m.%Y') BETWEEN '{$data1}' AND '$data2')) ";
        $find = trim(explode("#", $find)[2]);
    }

    if($find != null and $find != "" and $find != "undefined") {
        $fieldForSearch = "nomeretap, namecontr, contentwork, nomdog, datestart, datestop, otchet, prim";
...
$stmt = $pdo->prepare("SELECT COUNT(*) as total FROM contragent WHERE hide = 0 {$filter} AND detid = :detid");
...
$query = "SELECT *, {$sortfield} FROM contragent where hide = 0 {$filter} and detid = :detid ORDER BY {$sortirovka} LIMIT {$pageSize} OFFSET {$startLine}";
...
function findLinks($data, $pdo) {
    ...
    $query = "SELECT * FROM uplfiles where hide = 0 and tabname = 'contragent' and detid IN ($detidList)";
    $stmt = $pdo->prepare($query);
    $stmt->execute();
    $result = $stmt->fetchAll(PDO::FETCH_ASSOC);
