...
$stmt = $pdo->prepare("SELECT COUNT(*) as total FROM mailbox WHERE hide = {$hide} {$filter} and datecontrol != '' ");
$stmt->execute();
...
$query = "SELECT *, {$sortfield} FROM mailbox where hide = {$hide} {$filter} and datecontrol != '' ORDER BY {$sortirovka} LIMIT :pageSize OFFSET :startLine";
