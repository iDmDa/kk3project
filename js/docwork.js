class DocworkTableGenerator {

    constructor() {
        if(typeof DocworkTableGenerator.instance === 'object') {
            return DocworkTableGenerator.instance;
        }

        //this.fieldList = null;
        DocworkTableGenerator.instance = this;
    }
    ...
    createNumberLine() {
        ...
        for(let i = 0; i < len; i++) {
            ...
            td.classList.add("docwork-context");
            ...
            td.dataset.actfile = "docwork.php";
    ...
    createEvents() {
		...
		table.addEventListener("click", function(event) {
            ...
            if(event.target.classList == "button_field"){
                ...
                if(event.target.parentNode.id.split("_")[1] == "chernovik") type = 1;
        ...
        addButton.addEventListener("click", function(event) {
            ...
            xhr.open('POST', 'docwork.php', true);
            xhr.send(data);
            xhr.onload = function () {
    ...
    loadAllFileIcon() {
        ...
        xhr.onload = function () {
            ...
            resultArray = JSON.parse(resp);
            createIcons(resultArray, [['2', '_scan_docwork'], ['1', '_chernovik_docwork']]);
        }
    }
...
function docworkLoad (sendObject) {
    let data = new FormData();
    data.append("docwork", "value");
	...
	xhr.onloadend = function(event) {
    	...
    	console.log("(docworkLoad)Загрузка завершена");
        if(sendObject.find != "") findSelect(sendObject.find);
  	}
}

function docworkfindbox(tabid) {
